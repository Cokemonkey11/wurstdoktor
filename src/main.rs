...
fn name_generics<'a>() -> Parser<'a, u8, String> {
    (none_of(b" (.,\t\r\n<").repeat(1..)
        + (sym(b'<')
            + (list(
                is_a(|u: u8| u8::is_ascii_alphanumeric(&u)).repeat(1..), /* call(name_generics) */
                sym(b',') + sym(b' ').repeat(0..),
            ))
            + sym(b'>'))
        .opt())
    .collect()
    .convert(|st| String::from_utf8(st.to_vec()))
}
...
fn indented<'a>() -> Parser<'a, u8, Vec<u8>> {
    (seq(b"\t") | seq(b"    ")) * none_of(b"\n\r").repeat(1..) - linebreak()
}

fn cruft<'a>() -> Parser<'a, u8, ()> {
    (!class_declaration() * !free_function() * !eenum() * !interface() * take(1)).discard()
}
...
fn interface_cruft<'a>() -> Parser<'a, u8, ()> {
    (!class_fn() * take(1)).discard()
}

static PRIVATE: &[u8] = b"private";

fn class_fn<'a>() -> Parser<'a, u8, WurstFunction> {
    (doc() - maybe_whitespace()
        + (((
            // Only match non-private functions.
            !seq(PRIVATE)
                * seq(b"static ").opt().map(|p| {
                    p.map(|s| {
                        std::str::from_utf8(s)
                            .expect("Failed to unwrap a UTF8 string")
                            .to_string()
                    })
                })
                + ((seq(b"function ") * name_generics())
                    | seq(b"construct").map(|s| {
                        std::str::from_utf8(s)
                            .expect("Failed to unwrap UTF8")
                            .to_string()
                    }))
                - sym(b'(')
            ...
            | (seq(b"ondestroy")
                .map(|s| {
                    std::str::from_utf8(s)
                        .expect("Failed to unwrap UTF")
                        .to_string()
                })
                .map(|n| (((None, n), vec![]), None))))
        - maybe_whitespace())
    .map(|(d, (((s, n), p), r))| WurstFunction {
        doc: d,
        static_: s.is_some(),
        extensor: None,
        name: n,
...
> {
    doc()
        + (
            ...
            (seq(b"public ") * seq(b"abstract ").opt() - seq(b"class ")).map(|p| {
                p.map(|slice| {
                    std::str::from_utf8(slice)
                        .expect("Couldn't unwrap UTF8 string.")
                        .into()
                })
            }) + name()
                + (sep() * seq(b"extends") * sep() * name() - maybe_sep()).opt()
                + (sep()
                    * seq(b"implements")
                    * sep()
                    * list(name(), (sep() | one_of(b",").repeat(1..1)).repeat(1..)))
                .opt()
...
fn class<'a>() -> Parser<'a, u8, WurstClass> {
    (class_declaration()
        + class_cruft().repeat(0..) * list(call(class_fn), call(class_cruft).repeat(1..)))
...
> {
    (cruft().repeat(0..)
        * list(
            class().map(FunctionOrClassTemporary::Class)
                | free_function().map(FunctionOrClassTemporary::Function)
                | eenum().map(FunctionOrClassTemporary::Enum)
                | interface().map(FunctionOrClassTemporary::Interface),
            cruft().repeat(0..),
        )
        - cruft().repeat(0..))
    .map(|members| {
        members
            .into_iter()
            .fold((vec![], vec![], vec![], vec![]), |acc, x| match x {
                FunctionOrClassTemporary::Class(class) => (
                    acc.0.into_iter().chain(vec![class]).collect(),
                    acc.1,
                    acc.2,
                    acc.3,
                ),
                FunctionOrClassTemporary::Function(func) => (
                    acc.0,
                    acc.1.into_iter().chain(vec![func]).collect(),
                    acc.2,
                    acc.3,
                ),
                FunctionOrClassTemporary::Enum(eenum) => (
                    acc.0,
                    acc.1,
                    acc.2.into_iter().chain(vec![eenum]).collect(),
                    acc.3,
                ),
                FunctionOrClassTemporary::Interface(interface) => (
                    ...
                    acc.1,
                    acc.2,
                    acc.3.into_iter().chain(vec![interface]).collect(),
                ),
            })
    })
}
...
fn package<'a>() -> Parser<'a, u8, WurstPackage> {
    ...
    (
        ...
        match lines.len() {
            0 => package_contents().map(move |sub_parse| ((doc.clone(), name.clone()), sub_parse)),
            _ => {
                let sub_parse = package_contents().parse(&lines).expect("subparse");

                take(0).map(move |_nil| ((doc.clone(), name.clone()), sub_parse.clone()))
            }
        }
        .map(
            |((doc, name), (classes, free_fns, enums, interfaces))| WurstPackage {
                doc,
                name,
                classes,
                free_fns,
...
fn doc<'a>() -> Parser<'a, u8, Option<String>> {
    (maybe_emptylines()
        * seq(b"/**")
        * (!seq(b"*/") * take(1)).repeat(0..).collect().map(|v| {
            std::str::from_utf8(v)
                .expect("UTF8!")
...
fn function_params<'a>() -> Parser<'a, u8, Vec<WurstFnParam>> {
    maybe_whitespace()
        * list(
            // Parameters.
            none_of(b") ,").repeat(1..).convert(String::from_utf8) - sym(b' ').repeat(1..)
                + none_of(b") ,\r\n\t").repeat(1..).convert(String::from_utf8),
            sym(b',') * maybe_whitespace(),
        )
        .map(|v| {
            v.into_iter()
                .map(|(t, n)| WurstFnParam { typ: t, name: n })
                .collect()
        })
}
...
fn free_function<'a>() -> Parser<'a, u8, WurstFunction> {
    (doc() - maybe_whitespace()
        ...
        + (
            // Return value.
            maybe_sep() * seq(b"returns ") * none_of(b" ,\n").repeat(1..).convert(String::from_utf8)
        )
        .opt()
        - maybe_whitespace())
...
fn interface_body<'a>() -> Parser<'a, u8, Vec<WurstFunction>> {
    interface_cruft().repeat(0..) * list(call(class_fn), call(interface_cruft).repeat(0..))
}

fn interface<'a>() -> Parser<'a, u8, WurstInterface> {
    (doc() - maybe_whitespace() * seq(b"public interface") * sep()
        + name_generics()
        + (indented() | empty().map(|()| vec![]))
            ...
            .map(move |lines| interface_body().parse(&lines).expect("subparse!")))
...
fn wurstdok<'a>() -> Parser<'a, u8, WurstDok> {
    package().map(WurstDok::Package)
        | eenum().map(WurstDok::Enum)
        | class().map(WurstDok::Class)
        | interface().map(WurstDok::Interface)
        | free_function().map(WurstDok::FreeFunction)
}

fn wurstdoktor<'a>() -> Parser<'a, u8, Vec<WurstDok>> {
    (wurstdok() | !end() * take(1).map(|_| WurstDok::Nothing))
        .repeat(0..)
        .map(|v| v.into_iter().filter(|w| w != &WurstDok::Nothing).collect())
}
...
fn main() -> Result<(), Error> {
    ...
    let result: Result<(), Error> = match (opt.yaml, opt.sqlite, opt.sqlitedb) {
        (false, false, true) => {
                        ...
                        WurstDok::FreeFunction(func) => {
                            ...
                            for param in func.params {
                                connection.execute(
                                    ...
                                    &serde_rusqlite::to_params(&(id, param.typ, param.name))?
                                        .to_slice(),
                                )?;
                            }
                        }
        ...
        (false, true, false) => {
            ...
            println!("CREATE TABLE params (foreign_fn INTEGER, typ TEXT, name TEXT);");
        ...
        _ => Err(WurstdoktorError::BadOutputArguments.into()),
    };
...
mod tests {
    ...
    fn test_fv_game_timer() -> Result<(), ()> {
        assert_eq!(
            ...
            Ok(vec![WurstDok::Package(WurstPackage {
                doc: None,
                name: "GameTimer".into(),
                classes: vec![],
                enums: vec![],
    ...
    fn test_enum() -> Result<(), ()> {
        assert_eq!(
            ...
            Ok(vec![WurstDok::Enum(WurstEnum {
                doc: Some("Hello world!".into()),
                name: "Braap".into(),
                variants: vec!["_NONE".into(), "ONE".into(), "TWO".into(),]
            })])
        );
...
public function goth(unit g)
    ...
    fn test_two_fns() -> Result<(), ()> {
        assert_eq!(
            ...
            Ok(vec![
                WurstDok::FreeFunction(WurstFunction {
                    doc: Some("Okay.".into()),
                    static_: false,
                    extensor: None,
                    name: "braap".into(),
    ...
    fn test_pubfn() -> Result<(), ()> {
        assert_eq!(
            ...
            Ok(vec![WurstDok::FreeFunction(WurstFunction {
                doc: None,
                static_: false,
                extensor: None,
                name: "braap".into(),
...
function own() returns thistype
    ...
    fn test_class_extends() -> Result<(), ()> {
        assert_eq!(
            ...
            Ok(vec![WurstDok::Class(WurstClass {
                doc: None,
                abstract_: false,
                name: "A".into(),
